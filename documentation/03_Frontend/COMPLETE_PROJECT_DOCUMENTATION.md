...
The backend packaged in `Backend.zip` implements a small, focused FastAPI surface that the frontend currently uses. Key routes are listed below (base URL `http://127.0.0.1:8000`):
...
**Purpose:** Register a new driver profile (multipart form)
...
**Behavior:** Extracts a face embedding using InsightFace and stores the profile in a local SQLite `drivers.db` table (embedding saved as JSON string).

**Response:**
```json
{ "success": true, "message": "{name} registered" }
```

#### 3. `POST /recognize-driver`
**Purpose:** Identify a driver from an uploaded face image

**Request (multipart/form-data):**
- `file` (image/jpeg)

**Behavior:** Computes an embedding and compares against stored embeddings using cosine similarity (threshold ≈ 0.45). Returns match result when confidence is above threshold.

**Response (matched):**
```json
{ "matched": true, "driver": "Alex Driver", "confidence": 0.52 }
```

**Response (no match):**
```json
{ "matched": false, "message": "Unknown driver" }
```

#### 4. `POST /detect-face`
**Purpose:** Run driver face analysis (drowsiness, distraction, phone detection)

**Request (multipart/form-data):**
- `file` (image/jpeg)

**Behavior:** Uses MediaPipe FaceMesh, OpenCV, and a YOLOv8 model (for phone detection) to compute a `TelemetryResponse` (Pydantic model) containing `driver`, `vision`, `vehicle`, and `events` objects. This is the primary telemetry endpoint consumed by the frontend for live monitoring.

**Response:** Pydantic `TelemetryResponse` with nested `DriverTelemetry`, `VisionTelemetry`, `VehicleTelemetry`, and `events` (see `app/models/telemetry_models.py`).

#### 5. `DELETE /clear-drivers`
**Purpose:** Testing utility to clear all driver profiles from the local `drivers` table.

**Response:**
```json
{ "success": true, "message": "All driver profiles cleared" }
```

Note: The packaged backend does not implement `/telemetry` or `/profiles` REST resources as separate endpoints — telemetry is returned by `/detect-face` and driver profiles are stored in a local SQLite `drivers.db` table managed by `database.py`. Update frontend calls accordingly to use the implemented routes above.
